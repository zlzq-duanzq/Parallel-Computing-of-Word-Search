// g++ -o find serial_bfs_dfs_trie.cpp -std=c++11
...
void bfs(vector<vector<char>>& board, TrieNode *root, set<string> &result, queue<State>& bfs_queue){
...
set<string> findWords(vector<vector<char>> &board, vector<string> &words) {
    ...
    bfs(board, root, result, bfs_queue);
