...
set<string> findWords(vector<vector<char>> &board, vector<string> &words) {
    ...
    set<string> result;
    cout << "copying board each time, so no board lock" << endl;
    
    #pragma omp parallel for collapse(2)
    for (int i = 0; i < board.size(); i++) {
        for (int j = 0; j < board[0].size(); j++) {
            vector<vector<char>> newboard = board;
            dfs(newboard, i, j, root, result);
        }
    }
    return result;
}
...
// TrieNode *buildTrie_parallel(vector<string> &words) {
//     TrieNode *root = new TrieNode();
// #pragma omp parallel 
// 	{
// #pragma omp for
...
void dfs(char *board, int i, int j, TrieNode *p, set<string> &result) {
    char c = board[i][j];
